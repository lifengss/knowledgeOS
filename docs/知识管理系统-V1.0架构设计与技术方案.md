...
> **单一权威来源**：本文档是系统设计的唯一权威版本，随代码通过 Git 管理。父目录 `knowledgeOS/` 下同名文件仅为重定向索引，请勿在此之外重复维护设计文档。
...
| Raw 独立溯源区 | PRD/需求上传后原文独立存储于 `project-wiki/raw/`，Wiki 页可溯源查看 | ✅（V1.2） |
| 实体抽取闭环（GBrain） | Wiki 页触发 GBrain 语义抽取实体，自动生成实体索引页与实体内容页 | ✅（V1.2） |

### 1.4 V1.2 新增能力说明

V1.2 在 V1.0 知识闭环基础上新增两项与「项目 Wiki 源文档」相关的能力，详见第 13 章：

- **Raw 独立溯源区**：上传 PRD / 需求时，原文除沉淀为 Wiki 页面外，另存一份独立原始文档（`project-wiki/raw/`），支持在 Wiki 中溯源查看一手素材。
- **实体抽取闭环**：由 GBrain（已配置的 LLM endpoint）对 PRD / 需求做语义实体抽取，自动生成「实体索引页 + 各实体内容页」并沉淀进项目 Wiki，形成可检索、可图谱关联的知识骨架。
...
### 6.5 business-graph-builder（业务流程依赖知识图谱）

> 定位：运行期 **功能模块**（`skills/business_graph_builder.py`），不是编码智能体技能（放在 `.codebuddy/skills/` 的才是）。
> 它解决「业务步骤 / 依赖 / 场景」这类需要结合文档分析、纯确定性代码无法凭空臆造的语义问题。

**两层能力**

- Layer 1 生成与入库：从 project-wiki 的「架构/PRD」+「API 接口文档」出发，产出
  `business-flows.json`（结构化，含场景依赖全部信息）与 `业务流程与依赖图谱.md`（人类可读），
  写入当前项目的 `brains/<pid>/project-wiki/`（入库项目 Wiki）。
- Layer 2 服务：由 `GET /api/business-graph` 读取，供网页端渲染力导向图，
  以及业务前端（testcase-gen-frontend）在生成测试用例时取业务上下文。

**为什么需要 AI（而非纯确定性代码）**

业务依赖分析本质是「读文档 → 推理步骤/依赖/场景」的语义任务。纯 Python 只能做两件事：
1. 由 API 文档端点生成**骨架节点**（确定性，语义字段留空，待补全）；
2. 校验 / 由 JSON 生成 MD / 入库 / 读取。

因此重建走「**AI 业务依赖分析 + 确定性骨架兜底**」双路径：

- `generate`（主入口）：调用统一 AI 通道（`ai/ai_adapter.call_provider`，codebuddy/openai/none）
  基于架构与 API 文档产出完整图谱；AI 不可用或输出不合法时回退到骨架。
  可由 `POST /api/business-graph`（body `ai=false` 可强制仅骨架）触发「从文档重新生成」。
- `scaffold`：纯确定性骨架子命令。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/business-graph` | 读取已入库图谱；未入库时 `data` 为 `null` |
| POST | `/api/business-graph` | 从文档重新生成（AI 优先，确定性兜底），`body.sources` 可指定素材；返回 `{source, nodes, edges, flows, path}` |

**素材选择（Web UI）**：网页「业务流程依赖知识图谱」页「选择素材生成」按钮 → 弹框列出当前项目「项目 Wiki」页面（复选），确认后读取所选页面内容，以 `body.sources=[{id,title,content}]` 提交，仅基于所选素材生成（而非全量扫描）。适用于「只针对某几份文档重新建模业务」的场景。

**CLI 子命令**：`validate | md | scaffold | ingest | load | generate`（generate 支持 `--sources-file` 指定素材 JSON）。
...
| `POST /api/drafts/batch-commit` | 提交该项目 `pending/approved` 草稿；body 传 `ids:[...]` 仅提交选中项，否则全量提交，写入该项目私有库 |
| `DELETE /api/drafts/:id` | 删除单条草稿（同步清理其分解子页面） |
...
| `GET /api/projects` | 列出全部项目（含 default、sharedBrain、各项目私有库路径） |
| `POST /api/projects` | 运行时创建项目：追加 `config/projects.json` 条目并创建私有库目录 |
| `DELETE /api/projects/:id` | 运行时删除项目：禁删 default，清理私有库目录并写回配置 |
| `GET /api/brain/private-pages` | 仅列出当前项目私有库页面（不含共享库），供筛选晋升 |
| `POST /api/brain/promote` | 将私有页面复制/移动至共享库 `brains/_shared`，含路径越界校验与审计 |
| `DELETE /api/brain/pages/:category/:id` | 删除知识库单页（`id` 不含 `.md`），含路径越界校验与审计留痕 |
...
| `cache/draft_cache.py` | `project_id` 列与全部按项目过滤的查询；新增 `log-audit` 子命令为 promote 等运维操作留痕 |
| `api/projects.js` 新增 `addProject` / `removeProject` | 运行时增/删项目：写回 `config/projects.json`，`removeProject` 仅删私有库目录且禁止删除 default |
...
### 12.9 项目生命周期管理（运行时增删，V1.1 新增）

在"项目维度"之上，提供**运行时项目创建 / 删除**能力，使新增项目无需手工改配置与建目录。

#### 12.9.1 创建项目（`POST /api/projects` + `addProject`）

请求体：

```json
{ "id": "demo2", "name": "Demo2 项目", "description": "示例项目" }
```

- `api/projects.js` 的 `addProject({ id, name, description })`：
  - 自动计算私有库路径 `brainPath = brains/<id>`（默认）；
  - `fs.mkdirSync` 创建四个分类子目录（带 `.gitkeep`）；
  - 在 `config/projects.json` 的 `projects` 数组追加条目并写回；
  - 不重复创建已存在的项目（`findIndex` 校验）。
- 返回新增条目 `{ id, name, description, brainPath }`，前端 `loadProjects()` 刷新下拉并重算角标。

#### 12.9.2 删除项目（`DELETE /api/projects/:id` + `removeProject`）

- `removeProject(id)` 的约束：
  - **禁止删除默认项目**（`pid === config.defaultProject` 直接抛错），default 作为兜底不可移除；
  - 仅删除该项目**私有库目录**（`brains/<id>`），且强制校验目标路径以 `PROJECT_DIR` 为前缀，避免误删共享库或其他目录；
  - 从 `config/projects.json` 剔除条目并写回。
- 前端 `btn-delete-project` 仅在当前项目非 default 时可点，二次确认后调用删除接口，成功后自动切回 default 并刷新图谱缓存与分页。

#### 12.9.3 端点汇总（新增）

| 端点 | 行为 |
|------|------|
| `GET /api/projects` | 返回 `{ defaultProject, sharedBrain, projects:[{id,name,description,brainPath}] }`，供前端枚举与切换 |
| `POST /api/projects` | 运行时创建项目（写配置 + 建私有库目录） |
| `DELETE /api/projects/:id` | 运行时删除项目（禁删 default，清私有库目录 + 写回配置） |

### 12.10 私有知识晋升共享库（V1.1 新增）

为"共享库默认只读复用"提供**受控写入通道**：将项目私有知识库中的页面筛选后晋升到共享库 `brains/_shared`，使经验型知识跨项目复用。

#### 12.10.1 列出私有页面（`GET /api/brain/private-pages`）

仅扫描当前项目私有库（`resolveBrainDir(pid)`）的四个分类目录，**不含共享库**，返回：

```json
{ "success": true, "data": { "project": "demo", "pages": [
  { "category": "quality-rules", "filename": "x.md", "id": "x", "path": "quality-rules/x.md", "title": "...", "size": 1024, "mtime": "..." }
] } }
```

前端"晋升共享库"弹窗据此勾选待晋升页面。

#### 12.10.2 晋升（`POST /api/brain/promote`）

请求体：`{ "project": "<id>", "pagePath": "<分类>/<文件名.md>", "mode": "copy" | "move" }`。

安全与流程约束：

- `pagePath` 必填；拒绝包含 `..` 的路径穿越；拆分层级不足 2 级或首段分类不在 `CATEGORIES` 内均报错；
- 源文件必须在项目私有库目录内（`src.startsWith(brainDir)`）且真实存在，否则 404；
- `mode=copy`：`fs.copyFileSync` 复制到 `brains/_shared/<分类>/<文件>`（保留私有库原页）；
- `mode=move`：`fs.renameSync` 移动（从私有库移除）；
- 晋升成功后调用 `cache/draft_cache.py log-audit` 写入审计日志（`action=promote_copy|promote_move`，`target=<project>:<pagePath>`），审计失败不阻断主流程；
- 返回 `{ project, pagePath, mode, dest }`，晋升后共享页对所有项目在"全部"视图下可见。

## 13. V1.2 增补：Raw 溯源区与实体抽取闭环

> 本章为 V1.2 在 V1.0 知识闭环基础上新增的两项能力，均围绕「项目 Wiki 源文档」展开。

### 13.1 Raw 独立溯源区

**目标**：PRD / 需求上传并沉淀为项目 Wiki 页面后，仍保留一份「原始文档」独立存储，支持在 Wiki 中溯源查看一手素材，确保知识加工可回溯到原始输入。

**落库规则**（`POST /api/source-upload`，`type=prd|requirement`）：
- 主页面写入 `project-wiki/<slug>.md`，frontmatter 增加 `sourceFile`（上传原始文件名）与 `raw`（相对路径 `raw/<slug>-raw.md`）。
- 原文另存于 `project-wiki/raw/<slug>-raw.md`（独立 Raw 区，不参与 Wiki 索引分组）。
- 原文内容即上传提交的全文（含一级标题），不做二次加工。

**安全约束（读取隔离）**：Raw 溯源区为隔离存储，**不对外提供任何 API/MCP 读取**——`GET /api/brain/raw` 已移除，前端「查看原始文档」入口也已下线。此举避免长文档经 API/MCP 返回导致泄密与模型输入超长。原始全文仅由服务端内部流程直接读取（如 `POST /api/wiki/:category/:id/extract-entities` 抽取实体时读 `raw/<slug>-raw.md`），生成的结构化实体/概念内容页（如 `entity-*`）才对外可访问。

**查看入口（受控）**：原始文档溯源仅限服务端内部流程与受控运维，不在 Web UI / Agent MCP 暴露。

### 13.2 实体抽取闭环（GBrain 语义抽取）

**目标**：将 PRD / 需求文档中的关键概念、模块、流程、规则、接口等，由 GBrain（已配置的 LLM endpoint）语义抽取为结构化实体，自动生成「实体索引页」与「各实体内容页」，沉淀进项目 Wiki，形成可被检索与图谱关联的知识骨架。

**触发与流程**（`POST /api/wiki/:category/:id/extract-entities`）：
1. 读取源页面正文（剔除 frontmatter 与 mermaid 代码块）。
2. 调用 GBrain（复用 `ai/ai_config.py` 中 `gbrain` 段配置的 endpoint / apiKey / model）生成 JSON：`{"entities":[{"name","type","definition","attributes","relations","sourceSection"}]}`。
3. 解析 JSON（兼容 ```json 代码块包裹）后落库：
   - 实体内容页：`project-wiki/entity-<源id>-<实体slug>.md`，frontmatter 含 `type: entity`、`source`、`entityType`，正文含定义 / 属性 / 关联关系 / 出处（指向源文档章节）。
   - 实体索引页：`project-wiki/entity-index-<源id>.md`，列出全部实体并经由 `[[wikilink]]` 互链。
4. 返回实体数量与索引页 id。

**前端入口**：
- Wiki 文章页（PRD / 需求）渲染「抽取实体（GBrain）」按钮，点击后弹「处理中」提示（GBrain 调用通常需 1–2 分钟），完成后刷新左侧索引。
- 左侧索引新增「实体（GBrain 抽取）」分组，归集所有 `entity-*` 页面。

**设计要点**：
- GBrain 在知识系统中的定位是「LLM 推理端点」（复用 ai-summary / kb-qa 的调用通道），实体抽取是对其能力的进一步编排；抽取结果由本系统写回 `brains/`，GBrain 自身的 pglite 知识库不参与本流程。
- 实体抽取为「按需触发」而非自动：上传仅沉淀原文与页面，实体页由用户在 Wiki 页主动触发生成，避免无谓的 LLM 消耗。
- 实体 slug 采用安全清洗（`name.replace(/[^\w\u4e00-\u9fa5-]+/g,'-')`），避免 Windows 文件名非法字符导致写盘失败。
