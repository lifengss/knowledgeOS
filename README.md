...
| Python | >= 3.10 | 必需 |
| Git | >= 2.30 | 必需 |
| CodeBuddy CLI | 最新版 | **可选**，用于 `codebuddy` AI 通道 |
| OpenAI 兼容 API Key | — | **可选**，用于 `openai` AI 通道 |
| GBrain 内核 | — | **可选**，AI 规划/实体抽取增强，未安装时自动降级为确定性骨架 |

> **关于 GBrain 与 CodeBuddy 的说明**
>
> 本系统早期设计基于 GBrain 内核，但当前版本已解耦：
> - **不安装 GBrain 也能完整运行**，业务图谱、质量规则等功能会回退到基于 PRD/API 文档的确定性生成。
> - **不安装 CodeBuddy CLI 也能运行**，将 `data/ai_config.json` 中 `ai.provider` 设为 `none` 或 `openai` 即可。
> - 只有需要体验最佳 AI 生成效果时，才需要配置 CodeBuddy CLI 或 OpenAI 兼容 endpoint。
